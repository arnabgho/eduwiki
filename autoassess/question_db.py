...
def load_diagnose_question_set(
    ...
    if set_type == "Prereq":
        return load_diagnose_question_set_prereqs(
            topic, version,
    ...
    if autogenerated_question:
        try:
            questions.append(load_question(
                ...
                qtype=QUESTION_TYPE_WHAT_IS, question_shuffle=False))
            ...
            for qt in question_set.related_topics:
                try:
                    ...
                    question = load_question(
                        topic=qt, version=version,
                        quiz_topic=topic,
                        qtype=QUESTION_TYPE_WHAT_IS,
                        question_shuffle=question_shuffle)
                # print question
                questions.append(question)
        else:
            for q in question_set.questions:
                question = format_question_to_show(
                    q, answer_shuffle=question_shuffle)
                questions.append(question)
...
def load_diagnose_question_set_prereqs(topic, version, question_shuffle=False):
...
def load_question(
        topic, version,
        qtype=QUESTION_TYPE_WHAT_IS, quiz_topic=None,
        question_shuffle=True, verbose=False):
    """
    ...
    try:
        ...
        question = format_question_to_show(
            wiki_question, answer_shuffle=question_shuffle)
    except Exception as e:
        if verbose:
...
def format_question_to_show(wiki_question, answer_shuffle=True):
    ...
    if answer_shuffle:
        # print 'Answer Shuffled.'
        random.shuffle(possible_answers)

    choice_order = [a['idx'] for a in possible_answers]
