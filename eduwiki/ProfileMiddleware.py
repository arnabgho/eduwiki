...
class ProfileMiddleware(object):
        ...
        if settings.DEBUG:
            if 'time' in request.GET:
                ...
                if _middleware_start_time is None:
                    ...
                    self._start_rusage = resource.getrusage(
                ...
                if 'log' in request.GET and _log_file_path is None:
                    _log_file_path = get_log_file_path('middleware.profile',
                                                       time.gmtime())

    def process_view(self, request, callback, callback_args, callback_kwargs):
        if settings.DEBUG and ('prof' in request.GET):
            return _prof.runcall(callback, request, *callback_args,
                                 **callback_kwargs)
    ...
    def _display_time(self, request, response):
        ...
        if count > 1:
            stats_str += "%-4s %-24s %15.3f %14.3f %10.3f %12.3f %12.3f %12d %12d\n" % tuple(
                ['', 'Standard deviation'] + [stdev(values[field]) for field in
                                              time_fields])
