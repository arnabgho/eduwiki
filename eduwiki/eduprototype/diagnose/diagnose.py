...
#representing the term's article, with its prereqs and quiz items as well as those of 
#the prereqs, and of their prereqs, etc.
def query(search_term, depth = 1, children = 3):
    ...
    distractor_names = main_topic.wikilinks(children)    #string of text as the distractor
    distractors = []
    for i in range(0,3):
    ...
    def wikilinks(self, num):
        ...
        for m in wikilink_rx.finditer(wtext):
            ...
            if m.group(1) is not None:
                if "Image" in m.group(1) or "Template" in m.group(1) or "File" in m.group(1):
                    continue
                link_array.append(m.group(1)[:-1])
            else:
                if "Image" in m.group(2) or "Template" in m.group(2) or "File" in m.group(2):
                    continue
